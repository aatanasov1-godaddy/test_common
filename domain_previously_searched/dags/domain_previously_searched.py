...
### General Information
- Dataset Name         : domain_previously_searched
...
DAG_ID = 'domain_previously_searched'
...
PY_FILES = f'--py-files {CODE_PATH}/domain_previously_searched/spark/sql/*,{CODE_PATH}/utils/*'
CUSTOM_RENAME = (
                 '{"searched_utc_ts": '
                 '"searched_ts", '
...
with DAG(
    ...
    ingest_data_stg = RunEMRStepOperator(
        ...
        step_cmd=(
                  ...
                  '-d mdm_local '
                  '-t domain_previously_searched_history '
                  f'-e {ENV}'
                 ),
        trigger_rule='none_failed_min_one_success'
    )
    ...
    location = f'{s3_datalake}/domain_previously_searched/partition_utc_date={utcnow.strftime("%Y-%m-%d")}/'

    data_lake_success = SnapshotSuccessNotificationOperator(
        task_id='data_lake_success',
        db_name='marketing_egress_mart',
        table_name='domain_previously_searched',

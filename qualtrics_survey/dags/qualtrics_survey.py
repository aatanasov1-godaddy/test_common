...
PY_FILES = f'--py-files {CODE_PATH}/qualtrics_survey/spark/sql/*,{CODE_PATH}/utils/*'
CUSTOM_RENAME = (
                 '{"survey_response_end_utc_ts": '
                 '"survey_response_end_ts", '
...
with DAG(
    ...
    location = f'{s3_datalake}/qualtrics_survey/partition_utc_date={utcnow.strftime("%Y-%m-%d")}/'

    data_lake_success = SnapshotSuccessNotificationOperator(
        task_id='data_lake_success',
        db_name='marketing_egress_mart',
        table_name='qualtrics_survey',
